...
> Estado: `PROPUESTO`. Ninguna etapa se inicia sin validar antes las
...
- Tope de gasto y alerta de consumo en el panel de OpenAI.
...
- PostgreSQL, capa de migraciones, `organizations`, `users`, `memberships`,
  `api_keys`.
...
**Bloqueada por**: decisión de modelo de producto
([`business-model.md`](business-model.md)). Si se eligiera el Modelo B, buena
parte de esta etapa sobra.
...
- Extracción del puerto `AgentRuntime`
  ...
  la elección entre Agents y Responses pasa de variable global de proceso a
...
|---|---|---|
| La Agents API es beta y puede cambiar | Rotura del núcleo actual | Adelantar el puerto `AgentRuntime` de la Etapa 2 |
...
| Se elige el Modelo B | Etapas 1 y 4 se reducen mucho | Decidir el modelo antes de empezar la Etapa 1 |
| El coste de tokens hace inviable la membresía plana | Modelo económico roto | Medir desde la Etapa 0; "trae tu propia key" como salida |
